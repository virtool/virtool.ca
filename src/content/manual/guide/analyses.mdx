...
5. Use the mouse or the <Key>w</Key> and <Key>s</Key> keys to select OTUs

   <video src="/images/analyses/pathoscope_select.mp4" alt="Moving through pathoscope result hits." autoplay />

6. Click the <DummyButton icon="filter">Filter OTUs</DummyButton> button to show all OTUs
...
ORFs with significant HMM matches are not shown. This can be toggled using the
<DummyButton color="grey" icon="filter">Filter Sequences</DummyButton> button. In our
example, out of the 1072 assembled contigs, only 278 contained ORFs with significant
HMM matches.
...
default, ORFs with no significant HMM matches are not shown. This can be toggled using the
<DummyButton color="grey" icon="filter">Filter ORFs</DummyButton> button.
...
Contigs can be sent to NCBI for a BLAST search by clicking the <DummyButton color="grey" icon="cloud">BLAST at NCBI</DummyButton>
button. Many contigs derived from known viruses or non-viral sources will be assembled. It
is necessary to BLAST significant contigs to ensure they are novel.
